# Hi 👋, I'm Jeya Surya D S

🚀 Aspiring Software Engineer | Java Full-Stack Developer  
💻 Passionate about building scalable applications & solving real-world problems  
📍 Chennai, India  

---

## 🧠 About Me
- 🎓 B.Tech in Computer Science and Business Systems (CGPA: 8.2)
- 💡 Strong in Java, DSA, and Full-Stack Development
- ⚡ Experienced in building real-time and data-driven applications
- 🎯 Goal: Crack top tech roles and become a skilled Software Engineer

---

## 🛠️ Tech Stack

### 💻 Programming
Java | Python | JavaScript  

### 🌐 Web Development
HTML5 | CSS3 | React | Spring Boot  

### 🗄️ Databases
MySQL | PostgreSQL | MS SQL Server  

### 📊 Data Engineering
PySpark | Pandas | Databricks | Spark (SDP)  

### ⚙️ Tools
Git | GitHub | IntelliJ | VS Code | Postman  

### 📚 Core Concepts
OOP | REST APIs | MVC Architecture  

---

## 🚀 Projects

### 🔹 CampusConnect – Academic Collaboration Platform
- Full-stack app using **Spring Boot + Java**
- Real-time chat using **WebSockets**
- Features: mentorship, study groups, resource sharing
- Responsive UI with modern design (dark mode + glassmorphism)

🔗 https://github.com/Jeya-Surya/Campus-Connect  

---

### 🔹 Weather App – React Forecasting Application
- Built using **React + Open-Meteo API**
- Features: real-time weather, 7-day forecast, unit conversion
- Optimized performance and deployed on GitHub Pages

🔗 https://jeya-surya.github.io/React-Weather-App  

---

### 🔹 Data Warehouse & Lakehouse Pipeline
- Built using **SQL + Spark (SDP)**
- Implemented **Medallion Architecture**
- Integrated CRM & ERP datasets
- Applied dimensional modeling and transformations

🔗 https://github.com/Jeya-Surya/sdp-data-warehouse-project  

---

## 📜 Certifications
- Programming in Java – NPTEL  
- Spring Boot – Amigoscode  
- React – Scrimba  
- SQL – Skillrack  
- Internet of Things – NPTEL  
- Generative AI – LinkedIn Learning  

---

## 📫 Connect With Me

📧 Email: jeyasurya.ds08@gmail.com  
🔗 LinkedIn: https://linkedin.com/in/jeyasuryads  
💻 GitHub: https://github.com/Jeya-Surya  

---

## ⚡ Fun Fact
I enjoy building real-time applications and exploring both **software engineering + data engineering** 🚀
